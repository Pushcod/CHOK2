...
var timeToWaitForInput = 1000; //Задержка времени на проверку ответа (1 секунда)

async function loadQuiz() {
    try {
        const response = await fetch('quiz_data_2.xlsx');
...
function showQuestion() {
    ...
    if (question.Type.toLowerCase() === 'multi') {
...
async function nextQuestion() {
    if (isInputQuestion) {
        ...
        // Для типа multiinput просто переходим на следующий вопрос
        currentQuestion++;
